...
func connectDB(cfg *conf.Data_Database) *gorm.DB {
	if cfg == nil { //空指针
		panic(errors.New("GEN:connectDB fail need conf"))
	}
	switch strings.ToLower(cfg.GetDriver()) { //strings.ToLower 转为小写
	case "mysql":
...
func main() {
	...
	g := gen.NewGenerator(gen.Config{
		...
		FieldNullable: true, //delete_at是可以为空的，加上这个字段为true表示空指针，区分传零值或不传默认空值
	})
